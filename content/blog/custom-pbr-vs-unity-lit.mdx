...
下面是手写 Cook-Torrance BRDF 的实时调参 demo：拖 `roughness` 看高光宽窄，拖 `metallic` 看金属流 F0 = albedo，点预设按钮一键对比金 / 铜 / 塑料 / 铁的高光形态差异——上面的 α 平方链、Fresnel、能量守恒都在这个球上。

<PBRPlayground />

也可以直接编辑 fragment shader、拖滑块实时调参（2D 假球沙盒，可改代码）：
