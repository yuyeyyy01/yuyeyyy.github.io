/**
 * ShaderPlayground 预设 fragment 集合。
 * 与 project-shaders.ts 同配色：青绿 vec3(0.31,0.82,0.78) + 琥珀 vec3(0.91,0.69,0.29)。
 * 每个 fragment 控制在 25 行内，动效克制不闪烁。
 *
 * uniform 约定：iTime, iResolution；宏 uv 已在 ShaderPlayground prelude 提供。
 * 用户自定义 uniform 在 fragment 里自行声明，名字须与 UniformControl.name 一致。
 */

import type { UniformControl } from "./ShaderPlayground";
